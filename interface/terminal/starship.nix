...
in
{
  # We still use programs.starship if it exists, but let's be safe and just deploy config
  # and use the zsh hook.
  environment.systemPackages = [ pkgs.starship ];
  
  environment.etc."cafaye/terminal/starship.toml".text = (pkgs.formats.toml {}).generate "starship.toml" starshipSettings;

  # In Zsh init, we will hook it manually
}
