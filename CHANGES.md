...
- [x] Upgrade path from previous versions preserves state, configs, and workflows.
...
- [x] Entire environment can be reproduced from git state on a new machine.
...
- [x] Terminal navigation defaults are coherent for common engineering workflows.
...
- [x] First-time setup time is acceptable on low/medium compute VPS instances.
  - Verified on fresh GCP `e2-medium` (Ubuntu 24.04) with first install duration of ~210s and successful idempotent rerun.
...
- [x] Release announcement and upgrade guidance are prepared and reviewed.
  - Draft and upgrade path documented in `docs/RELEASE-ANNOUNCEMENT.md`.
